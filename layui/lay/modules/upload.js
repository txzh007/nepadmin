...
layui.define(["layer", "view", "conf"], function (exports) {
  "use strict";
  ...
  var $ = layui.$,
    ...
    thisUpload = function () {
      ...
      return {
        ...
        config: createOptions(that.config),
      };
    },
    //字符常量
    MOD_NAME = "upload",
  ...
  Class.prototype.upload = function (files, type) {
    var that = this,
      ...
      error = function (index) {
        ...
        typeof options.error === "function" &&
          options.error(index || 0, function (files) {
            that.upload(files);
          });
      },
      exts = options.exts,
    ...
    switch (options.accept) {
      ...
      case "audio": //音频文件
        if (
          !RegExp("\\w\\.(" + (exts || "mp3|wav|mid") + ")$", "i").test(
            escape(value)
          )
        ) {
